...
class PiorirityConsolidatedViewsTests(APITestCase):
    ...
    def test_url_pioririty_consolidated(self):
        ...
        response = self.client.get(self.url_pioririty_consolidated, {'start_date':self.parameters['start_date'],'end_date':self.parameters['end_date']})
    ...
    def test_url_pioririty_consolidated_stats(self):
        ...
        response = self.client.get(self.url_pioririty_consolidated_stats, {'start_date':self.parameters['start_date'],'end_date':self.parameters['end_date']})

        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_url_pioririty_consolidated_table(self):
        """Test url pioririty consolidated table."""
        response = self.client.get(self.url_pioririty_consolidated_table, {'start_date':self.parameters['start_date'],'end_date':self.parameters['end_date']})
...
class PiorirityConsolidatedFiltersViewsTests(APITestCase):
    ...
    def test_request_with_filters_is_valid(self):
        ...
        for i in range(len(response)):
            self.assertEqual(response[i]['co_cr'], self.parameters['co_cr'])
            self.assertEqual(response[i]['co_funai'], self.parameters['co_funai'])
            self.assertEqual(response[i]['no_estagio'], self.parameters['stage'])
            self.assertEqual(response[i]['prioridade'], self.parameters['priority'])
            self.assertTrue(
                datetime.strptime(self.parameters['start_date'], date_format)<=
                datetime.strptime(response[i]['dt_t_um'], date_format)<=
