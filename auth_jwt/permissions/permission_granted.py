...
permis_access_catalog = ('catalog.access_catalogs', 'catalog.access_satellite')

permis_catalog = permis_catalog_satellite + \
    permis_catalog_catalogs + permis_access_catalog
...
permis_access_funai = ('funai.access_limiteterraindigena',
                       'funai.access_coordenacaoregional')
permis_funai = permis_funai_limiteterraindigena + \
    permis_funai_coordenacaoregional + permis_access_funai
...
# permission = Permission.objects.create(codename="access_coordenacaoregional",
#                                        name="Can access coordenacaoregional FUNAI in CMR", content_type=content_type)

# content_type = ContentType.objects.get_for_model(models.LimiteTerraIndigena)
# permission = Permission.objects.create(codename="access_limiteterraindigena",
#                                        name="Can access limiteterraindigena FUNAI in CMR", content_type=content_type,)

# #######

# content_type = ContentType.objects.get_for_model(models.Catalogs)
# permission = Permission.objects.create(
#     codename="access_catalogs", name="Can acesso catalogs CATALOG in CMR", content_type=content_type,)

# content_type = ContentType.objects.get_for_model(models.Satellite)
# permission = Permission.objects.create(
#     codename="access_satellite", name="Can acesso satellite CATALOG in CMR", content_type=content_type,)

# Permission.objects.filter(content_type=content_type)
# #################################################################
# # grupos de permissões
# groups = ['FUNAI_SEDE', 'FUANI_COORDENACAO_REGIONAL', 'FUNAI_TERRA_INDIGENA',
#           'FPE', 'OUTRAS_INSTITUICOES', 'ACADEMICO', 'CULTURAL', 'AUTENTICADO']
...
#     """Função para adicionar permissões ao grupo"""
...
# #######
# add_permissions_in_group(group_name, permissions)
# create_new_users(list_users)
# add_user_in_group(group_name, list_users)
